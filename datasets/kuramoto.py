...
class Kuramoto(object):
    ...
    def kuramoto_ODE(self, t, y, arg):
        ...
        for m, _k in enumerate(k):
            ...
            phase += -np.sum(_k*np.sin((m+1)*dy),axis=1) # add -
...
def simulate_kuramoto(num_atoms, num_timesteps=10000, T=None, dt=0.01, undirected=False, intrinsic_freq=[]):
    if T is None:
    ...
    if len(intrinsic_freq)==0:
        intrinsic_freq = np.random.rand(num_atoms) * 9 + 1.
    else:
        intrinsic_freq = np.random.choice(intrinsic_freq, num_atoms, replace=False) 

    initial_phase = np.random.rand(num_atoms) * 2 * np.pi
    ratio_zero = 2/3#1/2#
    edges = np.random.choice(2, size=(num_atoms, num_atoms), p=[ratio_zero, 1-ratio_zero])
    if undirected:
    ...
    # Subsample
    phase_diff = np.diff(odePhi)[:, :] / dt # :10
    trajectories = np.sin(odePhi[:, 1:])[:, :] # :10-1
    cos_theta = np.cos(odePhi[:, 1:])[:, :] # :10-1
    init_phase_diff = np.expand_dims(phase_diff[:,0].copy(), 1)
    # np.concatenate([phase_diff[1:2,:].transpose(),(intrinsic_freq[1]-np.sin(odePhi[1:2, :-1]-odePhi[0:1, :-1])).transpose()],axis=1)
    ...
    cos_theta = np.expand_dims(cos_theta, -1)[:, :num_timesteps, :]
    ...
    init_phase_diff = np.expand_dims(np.repeat(
        init_phase_diff,
        num_timesteps, axis=1), -1)

    try:
        sim_data = np.concatenate(
            (phase_diff, trajectories, cos_theta, init_phase_diff, intrinsic_freq, phase),
            -1)
...
if __name__ == "__main__":
    ...
    plt.suptitle("Instantaneous freq / sin(theta)")
    plt.savefig('phases')
